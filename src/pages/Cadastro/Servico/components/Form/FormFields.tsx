    ...
    <Flex direction="column" justify="space-between">
      ...
      <Flex gap="3" align="center">
        <FormContainer label="Código" width="5rem">
          <Input maxLength={255} borderColor={colorMode === 'light' ? 'blackAlpha.600' : 'gray.600'} id="id" type="text" w="5rem" isReadOnly value={(`0000${isEditing ? editCod : cod}`).slice(-4)} {...register('nserv')} />
        </FormContainer>
        <FormContainer label="Descrição do Serviço" isRequired={true}>
          <Input maxLength={255} borderColor={colorMode === 'light' ? 'blackAlpha.600' : 'gray.600'} {...register('descricao')} />
        </FormContainer>
      </Flex>
      <Flex justify="space-between">
        <FormContainer label="Preço" width="6rem">
          <Input maxLength={255} borderColor={colorMode === 'light' ? 'blackAlpha.600' : 'gray.600'} placeholder="R$0,00" type="number" step={0.01} w="6rem" {...register('preco', {
            setValueAs: (value) => value === '' ? 0 : parseFloat(value),
          })} />
        </FormContainer>
        <FormContainer label="Unidade" width="5rem">
          <Input maxLength={255} borderColor={colorMode === 'light' ? 'blackAlpha.600' : 'gray.600'} w="5rem" {...register('un')} />
        </FormContainer>
      ...
      <Flex justify="space-between">
        <FormContainer label="Base de Calculo ISS" width="10rem">
          <Input maxLength={255} borderColor={colorMode === 'light' ? 'blackAlpha.600' : 'gray.600'} type="number" step={0.01} w="10rem" {...register('base_iss', {
            setValueAs: (value) => value === '' ? 0 : parseFloat(value),
          })} />
        </FormContainer>
        <FormContainer label="Alíquota ISS" width="10rem">
          <Input maxLength={255} borderColor={colorMode === 'light' ? 'blackAlpha.600' : 'gray.600'} type="number" step={0.01} w="10rem" {...register('aliquota_iss', {
            setValueAs: (value) => value === '' ? 0 : parseFloat(value),
          })} />
        </FormContainer>
        <FormContainer label="NCM" width="10rem">
          <Input maxLength={255} borderColor={colorMode === 'light' ? 'blackAlpha.600' : 'gray.600'} type="text" w="10rem" {...register('ncm')} />
        </FormContainer>
      </Flex>
      <Flex align="center" justify="space-between">
        <FormContainer label="Item Lista de Serviços" width="15rem">
          <Input maxLength={255} borderColor={colorMode === 'light' ? 'blackAlpha.600' : 'gray.600'} w="15rem" {...register('item_lista')} />
        </FormContainer>
